...
`demo -> 积木协议簇 -> 3D 积木市场 -> AI agent -> 3D引擎 -> XR引擎-> 元宇宙游戏引擎-> 无限Fate类游戏`
...
- 平台：macOS、Windows，编辑器形态为 `React + Vite + Tauri`
- 目标：验证 Door / TriggerZone / Switch / Ladder 的最小装配、交互、校验、回放链路
- 验证：`make check-m1`、`make check-m2`、`make check-m3`、`cargo test --manifest-path runtime/door_core/Cargo.toml`、`pnpm --dir editor/app run build`
