...
AI 只做工具：AI 只能产出可编辑的白盒结果（recipe/装配计划/参数建议）；不得进入运行时帧循环做决策。

可复现优先：同一 recipe + seed + lockfile 必须生成一致结果（跨机器、跨时间）。

语言收敛：第一阶段不得引入 C#；C++ 仅允许出现在渲染后端模块且必须可替换/可禁用。
...
桌面容器：Electron（第一阶段），后续可迁移 Tauri 但要 RFC
...
C++ 只允许位于 render/（或同级）模块；不得侵入 gameplay/world/asset 等核心领域。
...
runtime/（Rust）：游戏运行时、世界系统、积木执行、规则引擎、校验器核心

editor/（TS）：场景编辑、装配器、配方管理、可视化校验报告、调试可视化

render/（C++ 可选）：DX12/Vulkan/光追能力层（可编译开关）

protocol/（Rust+TS 共享定义）：消息协议/版本、JSON schema、错误码
...
推荐 ECS：Bevy ECS（不强制用 Bevy 引擎整套，但 ECS 思想必须）
...
只允许 C ABI（extern "C"）导出函数

禁止跨边界抛异常；C++ 内部必须捕获并返回错误码

只传递 POD（纯数据）：句柄/指针 + 长度，禁止传 C++ STL 容器

句柄生命周期必须由一侧负责（明确 ownership），不得“双方都能 free”

所有 API 必须带 api_version 与 struct_size（兼容演进）
...
schema-first：先改 schema，再改实现。schema 必须通过 CI 校验。
...
ECS/引擎壳：Bevy（MIT/Apache-2.0）

物理：Rapier（Apache-2.0）

序列化：Serde（MIT/Apache-2.0）

任务/并发：Tokio（MIT）

配置/Schema：JSON Schema（或等价）+ 自动校验
...
React（MIT）或 Vue（MIT）二选一（建议 React 生态更偏工具型）
...
Demo v0：只允许 Rust+TS，禁止引入 C++ 渲染后端

Alpha v0.5：允许引入 C++ 渲染后端，但必须可编译开关关闭

Beta v1：允许引入 Go Registry（线上生态），但不得反向污染 runtime/editor